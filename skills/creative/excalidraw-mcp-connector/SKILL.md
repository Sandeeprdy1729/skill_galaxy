...
```typescript
// Excalidraw MCP Server Implementation
...
server.tool(
  ...
  async ({ query, limit }) => {
    // Call Excalidraw JSON File Format
...
2. **Rate Limiting**: Implement request throttling to respect Excalidraw API rate limits
3. **Error Handling**: Provide clear, actionable error messages for common failure scenarios
4. **Pagination**: Handle paginated responses for large datasets efficiently
5. **Caching**: Cache frequently accessed read-only data to reduce API calls
6. **Security**: Validate all inputs before passing to the Excalidraw API; sanitize outputs
...
- Excalidraw Official Documentation
