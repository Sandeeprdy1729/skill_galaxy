...
/** Maximum trace length before switching to sampling instead of full subset enumeration. */
const MAX_TRACE_LEN_FOR_ENUMERATION = 12;

/** Multiplier for co-usage frequency when boosting recommendation scores. */
const CO_USAGE_BOOST_FACTOR = 0.1;

/** Maximum skills to process in one recommendation pass. */
const MAX_SKILL_POOL_SIZE = 500;

/* ── Helpers ────────────────────────────────────────────────────── */

/**
 * Convert a string to kebab-case, safe for use as a skill ID or filename.
 * @param {string} str
 * @param {number} maxLen  Maximum output length (default 80)
 * @returns {string}
 */
function toKebabCase(str, maxLen = 80) {
  return (str || 'untitled')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '')
...
function detectPatterns(traces, minSupport = 2, maxPatternSize = 4) {
  ...
  for (const trace of traces) {
    ...
    if (ids.length <= MAX_TRACE_LEN_FOR_ENUMERATION) {
    ...
    for (const item of enriched) {
      ...
      if (neighbors) {
        for (const selId of selectedIds) {
          ...
          score += freq * CO_USAGE_BOOST_FACTOR;
        }
      }
...
function synthesiseMetaSkill({ name, description, skills, category, difficulty }) {
  ...
  const kebabName = toKebabCase(name || 'meta-skill');
